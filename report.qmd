...
We used secondary data available online. The aggregated data of our base analyses on the spatial units of the analysis geometry is available at: <https://doi.org/10.4121/ca0da4ec-b4d0-430a-b8b9-84aa5a90446b.v1>.
...
We determined how many times more or less important is each criterion in relation to every other criterion. This was done in a subjective way, based on our understanding of the problem. First, we focused on mutual relations within the three themes (B, CA, QL), and then filled out the rest of the matrix and adjusted the matrix to be consistent. The complete matrix of the importance ratios looks like this:
...
![Overview of analysis geometry (by Viola Ebermannová)](img/AnalysisGeometry.png)
...
![Landcover overview (by Viola Ebermannová)](img/LandCover.png)

![Typical examples of land cover classes, draft estimate ecological quality indicator (by Viola Ebermannová)](/img/LandCover_typology.png)
...
We can also calculate the standard deviation for a better insight in the accuracy of the results. The code for this was written with the help of [Claude AI](https://claude.ai/). It is important to note that the standard deviation for attribute QL2acg in cluster 2 seems to be zero, but only because its cluster center also has an extremely low value. In this case, the standard deviation of zero therefore does not necessarily mean that the value is extremely accurate.
...
![Cluster overview in Senica (by Viola Ebermannová)](img/typology_clusteroverview.png)
...
Both B3ndvi and CA1temp score low, which means that there is not much need for cooling which is in line with a good quality of green in the area (B3ndvi). Type 3 areas have a good connectivity to existing ÚSES structures, so they are a good candidate for native biodiversity improvements. As for type 2, this can be done with small interventions. The high value for QL2acg is a bit unexpected, but can possibly be explained by the fact that important connective roads go through these areas, that naturally have high angular choice, more on that in the general clustering conclusion. These important roads might have a spatial potential to hav central functions, as many possible routes go through them, however their real-world centrality potential is low because of their location in relation to other urban functions. In a way, type 2 and type 3 do not differ that much from each other and we suspect that the differences are mostly caused by the presence of main roads, or absence of any roads. Type 2 and 3 will therefore have similar urban stream development priorities and strategies.
...
![MCDA Final conclusion (by Viola Ebermannová)](/img/MCDA_conclusions_sum_outline.png)
...
![Comparison of the 4 typologies and MCDA results (by Viola Ebermannová)](img/conclusion_MCDAxTypology.png)
...
Criteria should be comprehensive and measurable. Set of criteria should be complete, operational, decomposable, non-redundant, and minimal. Although we gave a lot of thought to the selection of criteria, we were limited by the availability of datasets, and also our analysis abilities. This limited the comprehensibility of the criteria, as by using land surface temperature to measure thermal comfort, or angular choice analysis to determine potential for central functions. Criteria *B2pot* and *B3ndvi* are partly redundant, because *B2pot* depends on land cover typology. In future research, the set of criteria should be reevaluated and individual criteria should be made more comprehensible.

### Typology construction and discussion on attibute *QL2acg*

We chose to select only 4 variables for the typology construction. This decision was made to limit the scatteredness of the resulting clusters, and better interpretability. For a more nuanced analysis, that would make justice to our morphologicaly defined quite detailed analysis geometry, it would be beneficial to use more variables, however we do expect the interpretation would be challenging. Moreover, to perform a more nuanced analysis and typology construction, the detail level of our data, and the preciseness in which it answers the research questions, would need to be higher. Ideally, we would have a larger number of more precise variables.

After we performed the clustering, the resulting clusters 2 and 3 were very similar in all variables other than *QL2acg*. In our opinion, our methodology for determining urban centralities is flawed, which caused this variable to not have much informational value. Therefore we came to the decision to group clusters 2 and 3 together for intervention recommendations.
...
An easier path towards a more telling analysis of potential centralities could be to adjust the radius of the angular choice analysis by slightly reducing it to better adhere to the scale of the city (which has a width of at the widest point ~ 2,3 km.) In addition, it could be good to include a larger area in the network map, because perhaps edge effect might have also had some effect on the ambiguity.

### Other notes

Although based in data, this research is subjective and depends on the authors' interpretation of data and results, as well as their values in determining the general research approach and priorities. Some parts of the research are especially subjective. Land cover typology classification was not based on much evidence, which is something that needs to be improved in possible future research. Also the Improvement potential was evaluated only conceptually and not based in evidence.
...
### Other notable flaws in analysis
...
In general, I do feel like this is a pretty big course and it is impossible to learn about everything. For example, the three workshops given in week 3. All of those workshops sounded so interesting but due to limited time it was sadly not feasible to do them all. Even though the workshops did play a significant part in the final analyses. Because I could only follow one of the workshops it was a bit hard for me to understand the results and workings of the analyses performed based on the other two workshops. As a team we could have put more effort into teaching and telling each other about what we learned in the different workshops, so that is something to keep in mind during future courses and group work.
...
#### Hongyue Kang’s reflection on teamwork

Through this course, I have developed a foundational understanding of both Multi-Criteria Decision Analysis (MCDA) and typology construction. While I did not directly participate in the typology construction process, I gained insight into how it works—particularly through learning about k-means clustering—and I find it to be a highly useful tool. I may apply this method in my future research. Importantly, I came to understand how typology construction and MCDA can complement each other: typologies help structure complex datasets into interpretable categories, while MCDA provides a framework to assess and make decisions based on these structured outcomes.

In this course, I was also responsible for the micro-climate analysis. Unfortunately, we did not obtain any concrete results, which I consider a real pity, as I am genuinely interested in this topic. The first challenge I encountered was data acquisition. Unlike in previous quarters, where datasets were prepared by tutors, this time I had to search for the data myself—a process that took considerable time. It was frustrating to find that most of the data was either imprecise or simply unavailable. Despite spending a great deal of time, I was unable to generate a meaningful outcome, which was quite discouraging. However, I also understand that this is a common reality in research: data is not always readily accessible. If I hope to conduct original research in the future, I must be prepared to face and overcome such obstacles.

The second difficulty I faced was the large deviation caused by the accumulation of multiple small errors. For example, when I attempted to calculate the shadow patterns in our research area, I lacked essential datasets such as DSM (Digital Surface Model) and CDSM. As a workaround, I tried to generate them using NDVI, DEM, and available building height data. However, this approach was problematic. The NDVI data only allowed for rough estimation of vegetation height and coverage, and the building height information was based on a 100×100 meter raster layer, which was far too coarse. These small inaccuracies compounded, resulting in a significant deviation in the shadow simulation. In the end, the outcome was unusable. If I had more time, I would have ensured that every dataset used was as accurate and high-resolution as possible. This is an important lesson I will carry forward into future research.

Overall, this course taught me scientific methods for conducting research and analyzing problems in a more quantitative way. I believe this is especially valuable in the fields of architecture and urban studies, where research is often qualitative. In my view, qualitative insights alone are not always convincing unless they are supported by reliable, empirical data.
