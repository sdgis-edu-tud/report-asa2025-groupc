...
We use MCDA to determine suitability of areas in Teplica river floodplain in the city of Senica for urban stream restoration measures targeting specific themes, and also in general for sustainable urban stream restoration measures.

### Selection of objectives and criteria – three pillar system

The approach of ReBioClim project builds on three pillars: _"The primary aim is to improve the quality of life in cities and at the same time to promote biodiversity and climate adaptation."_ (ReBioClim n.d.)

![Approach: combining the themes of biodiversity, climate adaptation, and quality of life](/img/_venndiagram.png){width="400"}
...
![Research structure – simplified flow chart of indicators used in each of the three themes](/img/MCDAresearchStructure_overview.png)

#### Detailed research structure

Each of the sub/themes comes with its own research subquestion. In the scheme below, you can see a more detailed look at the MCDA research structure, together with an overview of what data we use to evaluate the attributes, and what format of geometry that data is. More specifics of how we sourced the data, what it represents, and how we worked with it are described below in the chapters dedicated to individual attributes.


<iframe src="pdf/MCDA_researchStructure_detailed.pdf" width="100%" height="500" frameborder="0"></iframe>
...
In MCDA, each attribute results in a quantifiable criterion. These criteria are normalised and weighed among each other based on their hierarchy of importance in relation to the research question. There are different approaches to determined the weights, we used pairwise comparison Saaty matrix. For this, we used [Fuzzy AHP](https://fuzzyahp.holecekp.eu/en/info) by Pavel Holeček.
...
Consistency is acceptable with consistency ration CR = 0,086 (this is a mechanism to check if the researcher's assigning of importance ratios was consistent across the matrix):
...
This is a short overview of how MCDA works in this research project. We determined our research questions, and criteria that will describe them. We determined the mutual importance of the attributes and assigned the weights.

We divided the area into spatial units, on which we wanted to answer the research questions. For each of these units, we determined the value of each criterion. We normalised these criteria to 0-1, so they are comparable with each other.

Then, we multiplied the criteria by the weights to get the relative value (for each attribute, the sum of all the weighted criteria falls on 0-1). We calculated the sum of the criteria for each theme, and also a total sum for the complete MCDA value.

![Diagram of how MCDA works](img/placeholder.png)
...
ReBioClim. (n.d.) ReBioClim - ProjectOverview. Interreg Central Europe. Retriewed June 20, 2025 from <https://www.interreg-central.eu/projects/rebioclim/>
