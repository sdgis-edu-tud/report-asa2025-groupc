...
Kelly Schoonderwoerd (MSc Geomatics) – Data Analyst (surface temperature), R Expert, Clustering Lead

Viola Ebermannová (MSc Urbanism) – Project Coordinator, Research Lead, Design Lead (graphic guidelines and layout templates), Data Analyst (analysis geometry construction, space syntax analysis), MCDA Lead
...
Spatial analysis was conducted and maps produced by [QGIS](https://qgis.org/), an open source GIS (Geographic Information System) software. Typology construction was conducted through R scripts directly in the report file.
...
This is a short overview of how Multi Criteria Decision Analysis works in this research project. MCDA starts with a research question and subquestions, leading to objectives. Our research objectives are described further in the report. From these, we determined criteria that will be used to describe/answer them.
...
![Diagram of how MCDA works (by Viola Ebermannová)](img/MCDA_explanation.png)
...
![Map of ÚSES within our area (by Hongyue Kang)](/img/USES.png)
...
![NDVI classification (by Hongyue Kang)](img/NDVI.png)
...
To roughly estimate the need for cooling measures, we use Land Surface Temperature (LST) during a 3 month summer period.
...
![Soil potential (by Viola Ebermannová)](/img/CA_soil.png)
...
Aggregated as maximum value in polygon, to show the maximum potential in the area. Where there are no intersecting roads (gives result 'NULL'), 0 was assigned. This aggregate value was lineary normalised to 0-1 range.
...
Again, the value was aggregated by calculating maximum value in the polygon, to show the maximal potential in the area, and lineary normalised to 0-1 range.
...
An overall pairwise comparison matrix was created for all indicators at once (see more above). For comparing quality of life criteria among each other, the values were given as follows (accessible urban green has the overall highest priority, and QL1acg has more priority than QL2acg, because of central urban functions servicing more people):
...
When designing the weights, we gave highest priority to criteria that directly influence liveability and, in our opinion, offer very actionable basis for feasible and tenable investments (QL3gr, QL2acg, CA1temp). This approach is visible in the results: overall the highest score was achieved in well accessible urban areas, mainly in the east of the town.
...
We can also calculate the standard deviation for a better insight in the accuracy of the results. The code for this was written with the help of [Claude AI](https://claude.ai/).
...
The typologies created in Typology construction offer a simplified typology of areas that can act as a basis for an overview of typical spatial intervention sets.
...
In the area, there is a large water body present. This water body was assigned a high value for the nornalised *B3ndvi*, because water has a low NDVI value. That means it is classified as area with a large priority to improve the green cover based on its current state, despite there being no actuall green cover. This makes the analysis biased. <!-- -> In what way exactly???? What would we expect differently if we fixed this? -->
...
<iframe src="pdf/AngularChoice_20250518.pdf" width="100%" height="500" frameborder="0">
...
<iframe src="pdf/AttractionReach_GreenSpace_20250518.pdf" width="100%" height="500" frameborder="0">
...
<iframe src="pdf/AttractionReach_Teplica_20250518.pdf" width="100%" height="500" frameborder="0">
